...
#	endif
        ...
        m_Instance = create_instance(enabledLayers, enabledExtensions, enableDebugUtils);
        ...
        enumerate_devices(m_Instance);

        // Select appropriate Vulkan device.
        m_PhysicalDevice = select_device(m_Instance, m_Surface);
        ...
        if (auto const index = find_queue_family(m_PhysicalDevice, VK_QUEUE_GRAPHICS_BIT, m_Surface))
        {
        ...
        {
            auto graphics = find_queue_family(m_PhysicalDevice, VK_QUEUE_GRAPHICS_BIT);
            auto present = find_queue_family(m_PhysicalDevice, 0, m_Surface);
        ...
        m_Device = create_device(m_PhysicalDevice, queueFamilyIndices, enabledDevExensions);
        ...
        // Create swap chain
        std::tie(m_Swapchain, m_SwapchainFormat, m_SwapchainExtent) = create_swapchain(m_PhysicalDevice, m_Surface, m_Device, m_WindowHandle, queueFamilyIndices);
        
        // Get swap chain images & create associated image views
        get_swapchain_images(m_Device, m_Swapchain, m_SwapImages);
        create_swapchain_image_views(m_Device, m_SwapchainFormat, m_SwapImages, m_SwapViews);
